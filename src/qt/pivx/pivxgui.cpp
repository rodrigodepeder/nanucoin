...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Nanucoin Core client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Nanucoin Core"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Nanucoin Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
